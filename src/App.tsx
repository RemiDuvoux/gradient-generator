...
function sampleField(u: number, v: number, p: Params): number {
  const fx = u * p.freq;
...
const rgbStr = (c: RGB): string => `rgb(${c.r | 0},${c.g | 0},${c.b | 0})`;
...
function rampLookup(t: number, rgb: RGB[], stops: number[]): RampLookup {
...
function contrastF(f: number, contrast: number): number {
...
const bayer = (cx: number, cy: number): number =>
...
function buildStops(n: number, seed: number): number[] {
  if (n <= 1) return [0, 1];
  const stops = [0];
...
const TEXTURES: { id: TextureId; label: string }[] = [
